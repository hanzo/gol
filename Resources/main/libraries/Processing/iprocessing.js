...
// new code ##############################################################
this.Processing = function Processing( aElement, aCode, aPlatform ) {
	if ( typeof aElement == "string" )
		aElement = document.getElementById( aElement );

	var p = buildProcessing( aElement, aPlatform );
...
function buildProcessing( curElement, aPlatform ){       
  ...
  p.mouseButton = 0;
  p.mouseDown = false;
  p.pmouseButton = 0; // new code #### previous mouse button
  ...
  p.setup = undefined;
	...
	// new code ##############################################################
	var autoHideDefaultScreen = true;
	var touchList = [null,null];																												  
	// Global vars for tracking touch position and gesture values
  p.PORTRAIT = 1;
	p.LANDSCAPE = -1;
	p.ptouch1X = null;
	p.ptouch1Y = null;
	...
	p.touch2X = null;
	p.touch2Y = null;
	p.pgestureScale = null;	
	p.pgestureRotation = null;																													
	p.gestureScale = null;	
	p.gestureRotation = null;	
  p.orientation = p.PORTRAIT;																									
	// Will be replaced by the user, most likely																											  
	p.touch1Moved = undefined;
	p.touch1Started = undefined;
	p.touch1Stopped = undefined;
	p.touch2Moved = undefined;
	p.touch2Started = undefined;
	p.touch2Stopped = undefined;																												  
	p.gestureChanged = undefined;
	p.gestureStarted = undefined;
	p.gestureStopped = undefined;		
  p.shakeEvent = undefined;	
  p.orientationChanged = undefined;
  p.locationEvent = undefined;																													
	// #######################################################################	
  ...
  p.loadImage = function loadImage( file ) {
	...
	*/
	...
	// new code ##############################################################
	p.loadImage = function loadImage( f ) {
		if (platform == "web") {
			var filelist = p.split(f, "/");
			var file = filelist[filelist.length-1];
			var filepath = "data/";
			for ( var i = 0; i < filelist.length-1; i++ ) {
				filepath = filepath + filelist[i] + "/";
		  }
		  filepath = filepath + filelist[filelist.length-1];
			...
			data.name = file;
			data.img = img;
			if (newimage) { data.loaded = false; } else { data.loaded = true; }
    } else if (platform == "desktop" || platform == "iphone"){
			var filelist = p.split(f, "/");
			var file = filelist[filelist.length-1];
			var filepath = "data/";
			for ( var i = 0; i < filelist.length-1; i++ ) {
			...
			var h = img.height, w = img.width;
			var data = p.createImage( w, h );
			data.name = file;
			data.img = img;
			if (newimage) { data.loaded = false; } else { data.loaded = true; }
    }

    // fills width and height properties after image has loaded when using dynamic image elements
    // note: width and height are 0 until image has loaded, so main code cannot use these immediately 
    // warning: dynamic image elements may cause problems for buildImageObject(), createImage(), pixels array etc., need to look more at this 
    img.onload = function () {
      if (data) {																								  
	...
	// new code ##############################################################	
	p.loadSound = function loadSound( file ) {	
		if (platform == "web" || platform == "desktop") {
			var filepath = "data/" + file;
			
			var snd = document.getElementById(file);
			if ( !snd ) {
				snd = document.createElement("embed");
				snd.setAttribute("id", file);
				snd.setAttribute("src", filepath);
				snd.setAttribute("autostart", false);
				snd.setAttribute("loop", false);
				var datadiv = document.getElementById("data");
				datadiv.appendChild(snd);
      }
      ...
      data.loop = function(n) {
        if (n) {
          data.sound.setAttribute("loop", true);
				  data.sound.Play();
				} else {
          data.sound.setAttribute("loop", true);
				  data.sound.Play();
			...
			data.filename = file;
			data.filepath = filepath;
			data.sound = Titanium.Media.createSound(filepath);
			...
			data.loop = function(n) {
				if (n) {
				  data.sound.setLooping(true); // need to add a way to loop n number of times
          data.sound.play();
				} else {
				  data.sound.setLooping(true);
          data.sound.play();
				}
			};
			...
			data.stop = function() {
			  data.sound.stop();
        data.sound.setLooping(false);
			};
            
      // extended functions for iPhone platform ---------------------------------------
            
      data.rewind = function() {
			  data.sound.reset();
			};
            
      data.setVolume = function(f) {
			  data.sound.setVolume(f);
			};
            
      data.getVolume = function() {
			  return data.sound.getVolume();
			};
            
      data.isPlaying = function() {
			  return data.sound.isPlaying();
			};
            
      data.isLooping = function() {
			  return data.sound.isLooping();
			};
            
      data.isPaused = function() {
			  return data.sound.isPaused();
			};
	...
	p.loadFont = function loadFont( name ) {
		if(name.indexOf(".svg")==-1){
			return {
				name: name,
				width: function( str ) {
				if ( curContext.measureText )
					return curContext.measureText( typeof str == "number" ? String.fromCharCode( str ) : str ) / curTextSize;
					else if ( curContext.mozMeasureText )
  ...
  */
	...
	p.text = function text( str, x, y ) {
		if(!curTextFont.glyph){																																																							
			if (str && curContext.fillText ) {
				curContext.save();	  
				curContext.translate(x, y);
				curContext.font = curTextSize + "px " + curTextFont.name;
				curContext.fillText(typeof str == "number" ? String.fromCharCode( str ) : str, 0, 0);
				curContext.restore();
			} else if ( str && curContext.mozDrawText ) {
				curContext.save();
				curContext.mozTextStyle = curTextSize + "px " + curTextFont.name;
				curContext.translate(x, y);
  ...
  p.loadGlyphs=function loadGlyph(file){ // TH - changed (url)
			var url = "data/"	+ file;					 // TH - added line																																																	
      // SJAX SVG as XML D.O.
  ...
  */
	...
	p.lnPrinted = function lnPrinted(){};
	p.printed = function printed(){};  
	p.printlnConsole = function printlnConsole(){
		var Caller = arguments.callee.caller.name.toString();
		if ( arguments.length > 1 ) {
			Caller!="print"?
			p.ln = arguments:
	...
	p.println = function println( message, l ) {																																																			
		if (l == null){ var local = false; } else { var local = l; }
		if (platform == "iphone") {
			if (local) {
  ...
  p.sqrt = function sqrt( aNumber ) {
	...
	// new code ##############################################################																																																								
	p.value = function value( aString ) {
	  return parseInt( aString )
	};
	
	p.boolean = function boolean( aNumber ) {
  ...
  p.updatePixels = function() {
	...
	// NEW CODE - IPHONE @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@                                                                                                                                                                                                                             
	p.IPhone = function IPhone() {
		if (platform == "iphone") {
			var ip = {};
      ip.name = Titanium.Platform.username;				
      ip.device = Titanium.Platform.model;
      ip.os = Titanium.Platform.name;
      ip.version = Titanium.Platform.version;
      ip.uuid = Titanium.Platform.id;
			ip.latitude = null;
			ip.longitude = null;
			ip.altitude = null;																								
			...
			ip.preventHideDefaultScreen = function() {
			  autoHideDefaultScreen = false;
			};
			
			ip.hideDefaultScreen = function() {																												
				var defaultDiv = document.getElementById("default");										
				if (defaultDiv) { 
					defaultDiv.style.height = '0px';
					defaultDiv.style.width = '0px';
					defaultDiv.style.visibility = 'hidden';
				}
			};
			
			ip.showDefaultScreen = function() {																								
				var defaultDiv = document.getElementById("default");										
				if (defaultDiv) { 
					defaultDiv.style.height = '460px';
					defaultDiv.style.width = '320px';
					defaultDiv.style.visibility = 'visible';
				}
			};
			
			ip.getInfo = function() {																										  
        var info = ip.name + "," + ip.device + "," + ip.os + "," + ip.version + "," + ip.uuid;
        return info;
			};
      ...
      ip.loadSound = function loadSound( file ) {	
        ...
        return p.orientation;
			...
			var locationWatcher;																										  
			ip.startLocation = function() {	
        locationWatcher = Titanium.Geolocation.watchPosition(
          function(loc) {  
          ...
          function() { 
			...
			ip.stopLocation = function() {
				if (locationWatcher) {																									  
				  Titanium.Geolocation.clearWatch(locationWatcher);
				}
				ip.latitude = null;
				ip.longitude = null;
				ip.altitude = null;																										
			...
			ip.getLocation = function() {																							  
				var loc = {};																										  
        loc.latitude = ip.latitude;
        loc.longitude = ip.longitude;
        loc.altitude = ip.altitude;
        return loc;
			};	
      ...
      ip.getCurrentLocation = function() {
          Titanium.Geolocation.getCurrentPosition(
            function(loc) {
              var coords = loc.coords;
              var currentloc = {};																										  
              currentloc.latitude = coords.latitude;
              currentloc.longitude = coords.longitude;
              currentloc.altitude = coords.altitude;
              if ( p.locationEvent ) {
                  p.locationEvent(currentloc);
              }
            }, 
            function() {
        ...
        accelerometerWatcher = Titanium.Accelerometer.addEventListener('update',function(acc)
        {
          ip.accelerationX = acc.x;
          ip.accelerationY = acc.y;
          ip.accelerationZ = acc.z;
        });                                                                             
			};	
			
			ip.stopAccelerometer = function() {
				if (accelerometerWatcher) {																									  
				  Titanium.Accelerometer.removeEventListener('update',accelerometerWatcher);
				}
				ip.accelerationX = null;
				ip.accelerationY = null;
				ip.accelerationZ = null;
			...
			ip.getAcceleration = function() {																						  
				var acc = {};
        acc.x = ip.accelerationX;
        acc.y = ip.accelerationY;
        acc.z = ip.accelerationZ;
        return acc;
			};	
			
			ip.saveState = function(data) {		
        Titanium.App.Properties.setString('appState',data);
			}
			
      ip.loadState = function() { 
        return Titanium.App.Properties.getString('appState');
      };
			ip.getState = function() {	// for backwards compatibility
        return Titanium.App.Properties.getString('appState');
			};
  ...
  p.init = function init(code){
    ...
    // new code ##############################################################															
    if(platform == "iphone" && autoHideDefaultScreen) {
      // hide Default.png loading image	after setup()		
      var defaultDiv = document.getElementById("default");										
      if (defaultDiv) { 
          defaultDiv.style.height = '0px';
          defaultDiv.style.width = '0px';
    ...
    if (platform == "web" || platform == "desktop") {																																								
      ...
      attach( curElement, "mousedown", function(e) {
        mousePressed = true;
        p.pmouseButton = p.mouseButton; // new code ###
        switch(e.which){
    ...
    // new code ##############################################################
      ...
      attach( curElement, "touchmove", function(e) {
         ...
         for(var i = 0; i < e.changedTouches.length; i++) {
           if (e.changedTouches[i] == touchList[0]) { 	  	   
             p.ptouch1X = p.touch1X;
             p.ptouch1Y = p.touch1Y;
             p.touch1X = touchList[0].clientX;
             p.touch1Y = touchList[0].clientY;
             p.pmouseX = p.ptouch1X;
             p.pmouseY = p.ptouch1Y;
             p.mouseX = p.touch1X;
             p.mouseY = p.touch1Y;
             ...
             p.ptouch2X = p.touch2X;
             p.ptouch2Y = p.touch2Y;
             p.touch2X = touchList[1].clientX;
             p.touch2Y = touchList[1].clientY;
      ...
      attach( curElement, "gesturechange", function(e) {
         ...
         p.pgestureScale = p.gestureScale;	
         p.pgestureRotation = p.gestureRotation;	
         p.gestureScale = e.scale;
         p.gestureRotation = e.rotation;   
